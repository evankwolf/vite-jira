...
const TsReactTest = () => {
  ...
  return (
    <div>
      {/* 期待: 点击以后增加 john */}
      <button type="button" onClick={() => add({ name: "john", age: 22 })}>add john</button>
      {/* 期待: 点击以后删除第一项 */}
      <button type="button" onClick={() => removeIndex(0)}>remove 0</button>
      {/* 期待：点击以后清空列表 */}
      <button type="button" style={{ marginBottom: "50px" }} onClick={() => clear()}>
        clear
      </button>
      {value.map((person, index) => (
        <div key={Math.floor(Math.random() * 10000)} style={{ marginBottom: "30px" }}>
